...
export function TopBar({ me, weekStart, onPrev, onNext, onToday }: TopBarProps) {
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.defaultPrevented || e.metaKey || e.ctrlKey || e.altKey || e.shiftKey || isTyping(e.target)) return;
  ...
  return (
    <header className="glass flex h-14 shrink-0 items-center gap-2 px-3 sm:gap-3 sm:px-4">
      ...
      <h1 className="min-w-0 truncate text-sm font-medium whitespace-nowrap sm:text-lg" aria-live="polite">
      ...
      <div className="ml-auto flex shrink-0 items-center gap-2.5">
        ...
        <span className="hidden text-[13px] font-medium md:inline">{me.name.split(" ")[0]}</span>
        <form action={signOutAction}>
          <button type="submit" className="btn btn-ghost text-muted" aria-label="Sign out" title="Sign out">
