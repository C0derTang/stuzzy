...
const run = (day: number, startRow: number, endRow: number, free: string[]): Run => ({ day, startRow, endRow, free });
const PAST = 0;
...
describe("bestTimes", () => {
  ...
  it("excludes single-person runs", () => {
    expect(bestTimes([run(1, 20, 28, ["a"])], weekStart, PAST)).toEqual([]);
  });

  it("excludes windows shorter than an hour", () => {
    expect(bestTimes([run(1, 20, 21, ["a", "b"])], weekStart, PAST)).toEqual([]);
  });

  it("converts rows to instants", () => {
    expect(bestTimes([run(2, 28, 31, ["a", "b"])], weekStart, PAST)).toEqual([
      { start: at(2, 14), end: at(2, 15, 30), free: ["a", "b"] },
    ]);
  });

  it("ends at the next midnight when the window reaches the end of the day", () => {
    expect(bestTimes([run(0, 44, 48, ["a", "b"])], weekStart, PAST)).toEqual([
  ...
  it("does not extend across a gap or into another day", () => {
    const runs = [run(1, 18, 19, ["a", "b"]), run(1, 20, 21, ["a", "b"]), run(2, 19, 20, ["a", "b"])];
    expect(bestTimes(runs, weekStart, PAST)).toEqual([]);
  });

  it("reports one window per stretch, however many seeds reach it", () => {
    const runs = [run(3, 20, 22, ["a", "b"]), run(3, 22, 23, ["a", "b", "c"]), run(3, 23, 26, ["a", "b"])];
  ...
  it("keeps a bigger group's window inside a longer, smaller group's window", () => {
    const runs = [run(3, 20, 22, ["a", "b"]), run(3, 22, 26, ["a", "b", "c"])];
    expect(bestTimes(runs, weekStart, PAST)).toEqual([
  ...
  it("drops windows that are already over, keeps ones in progress", () => {
    const runs = [run(1, 18, 20, ["a", "b"]), run(1, 28, 32, ["a", "b"])];
  ...
  it("respects the limit", () => {
    const runs = Array.from({ length: 7 }, (_, day) => run(day, 20, 24, ["a", "b"]));
